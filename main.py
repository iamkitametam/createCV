...
def create_JSON_from_html(hh_link):
    ...
    url = ""
    try:
        ss = s.findAll("a", {"data-qa": "resume__invite-button"})
        q = ss[0].get_attribute_list("href")[0]
        url = "https://www.hh.ru/resume/" + q[q.find("r=")+2:]
    except:
        url = ""
    ...
    hh_key = url.replace("https://www.hh.ru/resume/","")

    # photo ###########################################################################################

    photo_url = ""
    photo_file_name = "imgs/" + FIO.upper() + "_" + hh_key + ".jpg"
    ...
    try:
        ss = s.findAll("span", {"itemprop": "telephone"})
        phone_numbers = ""
        for q in ss:
            phone_numbers = phone_numbers + q.text + ";"
    except:
        phone_numbers = ""

    try:
        ss = s.findAll("a", {"itemprop": "email"})
        emails = ""
        for q in ss:
            emails = emails + q.text + ";"
    except:
        emails = ""
    ...
    try:
        with open("txts/" + FIO + ".txt", 'w', encoding='utf-8') as outfile:
            json.dump(person, outfile, ensure_ascii=False)
    except:
        print("json saving failed!")
...
if (x > y):
    person = create_JSON_from_html(hh_link)
    create_CV_using_my_word_template(person)
else:
